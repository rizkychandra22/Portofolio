            ...
            <div class="row gy-4">
                ...
                <div class="col-lg-8">
                    <div class="portfolio-details-slider swiper init-swiper">
                        <script type="application/json" class="swiper-config">
                            {
                                "loop": {{ ($project->images ? $project->images->count(): 0) + 1 > 3 ? 'true' : 'false' }},
                                "speed": 500,
                                "autoplay": {
                                    "delay": 5000
                                },
                        ...
                        <div class="swiper-wrapper align-items-center">
                            ...
                            <div class="swiper-slide">
                                <a href="{{ \Illuminate\Support\Facades\Storage::disk('cloudinary')->url($project->image_project) }}" class="glightbox" data-gallery="portfolio-gallery">
                                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('cloudinary')->url($project->image_project) }}" crossorigin="anonymous" alt="{{ $project->{'name_project_' . app()->getLocale()} ?? '' }}" class="img-fluid" fetchpriority="high" decoding="async">
                                </a>
                            </div>

                            {{-- Menampilkan Gambar Tambahan dari Tabel portofolio_images --}}
                            @foreach($project->images as $galeri)
                                <div class="swiper-slide">
                    ...
                    <div class="portfolio-info" data-aos="fade-left" data-aos-delay="200">
                        ...
                        <ul>
                            <li><strong>{{ app()->getLocale() == 'id' ? 'Kategori' : 'Category' }}</strong>: 
                                {{ $project->category->{'name_category_' . app()->getLocale()} ?? '' }}
                            </li>
                            <li><strong>{{ app()->getLocale() == 'id' ? 'Nama Proyek' : 'Project Name' }}</strong>: 
                                {{ $project->{'name_project_' . app()->getLocale()} ?? '' }}
                            </li>
                            <li><strong>{{ app()->getLocale() == 'id' ? 'Tanggal Proyek' : 'Project Date' }}</strong>: 
                                {{ \Carbon\Carbon::parse($project->date_project)->locale(app()->getLocale())->translatedFormat('d F Y') }}
                            </li>
                            <li><strong>{{ app()->getLocale() == 'id' ? 'Link Proyek' : 'Project Link' }}</strong>: 
                                @if($project->link_project)
                                    <a href="{{ $project->link_project ?? '' }}" target="_blank" rel="noopener noreferrer" class="btn-link text-primary" aria-label="{{ app()->getLocale() == 'id' ? 'Buka tautan proyek' : 'Open project link' }}">
                            ...
                            <li><strong>{{ app()->getLocale() == 'id' ? 'Link Demo' : 'Demo Link' }}</strong>: 
                                @if($project->link_demo_project)
                                    <a href="{{ $project->link_demo_project ?? '' }}" target="_blank" rel="noopener noreferrer" class="btn-link text-primary" aria-label="{{ app()->getLocale() == 'id' ? 'Buka tautan demo' : 'Open demo link' }}">
                ...
                <div class="col-lg-12">
                    <div class="portfolio-description" data-aos="fade-up" data-aos-delay="300">
                        ...
                        @forelse($project->descriptions->where('type', 'overview') as $desc)
                            {{-- <p>{!! nl2br(e($desc->{'content_' . app()->getLocale()})) !!}</p> --}}
                            {!! $desc->{'content_' . app()->getLocale()} !!}
                        @empty
                            @if(app()->getLocale() == 'id')
                                <p>Deskripsi proyek <code>{{ $project->name_project_id }}</code> belum diunggah.</p>
                            @else
                                <p><code>{{ $project->name_project_en }}</code> project description not yet uploaded.</p>
                            @endif
                        @endforelse
                        ...
                        <div class="features mt-4">
                            ...
                            <div class="row gy-3">
                                @forelse($project->descriptions->where('type', 'feature') as $feature)
                                    <div class="col-md-6">
                                ...
                                @empty
                                    <div class="col-md-6 text-center">
                                        <div class="feature-item" data-aos="fade-up" data-aos-delay="400">
                                            ...
                                            @if(app()->getLocale() == 'id')
                                                <p>Fitur proyek <code>{{ $project->name_project_id ?? '' }}</code> belum diunggah.</p>
                                            @else
                                                <p><code>{{ $project->name_project_en ?? '' }}</code> project feature not yet uploaded.</p>
                                            @endif
                                        </div>
                                    </div>
                                @endforelse
